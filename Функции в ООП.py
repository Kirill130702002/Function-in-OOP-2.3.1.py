...
class DataSet:
    ...
    def filter_rows(self):
        ...
        self.vacancies_objects = list(filter(lambda vacancy: self.conditions_to_sort[self.filter_param[0]](vacancy, self.filter_param[1]), self.vacancies_objects))
    ...
    def sort_rows(self):
        ...
        """
        if self.sort_param != '':
            self.vacancies_objects.sort(key=lambda a: getattr(a, DataSet.ru_en_keys_dictionary[self.sort_param]), reverse=self.sort_reverse)
    ...
    def get_range(self):
        ...
        for index, vacancy in enumerate(self.vacancies_objects):
            if (length > 1 and self.sort_range[0] <= index < self.sort_range[1]) or (length == 1 and self.sort_range[0] <= index) or length == 0:
...
class InputConnect:
    ...
    def parse_table_fields(self, table_fields):
        ...
        """
        return InputConnect.table_header if table_fields == '' else ['№'] + [a for a in table_fields.split(', ') if a in InputConnect.table_header]
